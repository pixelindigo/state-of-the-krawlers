...
public class RTED_InfoTree_Opt {
	...
	public void computeOptimalStrategy() {
		...
		for (int v = 0; v < size1; v++) {
			...
			for (int w = 0; w < size2; w++) {
				...
				long[] mins = { leftMin, rightMin, heavyMin, Long.MAX_VALUE, revLeftMin,
						revRightMin, revHeavyMin };
		...
		for (int i1 = 1; i1 <= i - ioff; i1++) {
			for (int j1 = 1; j1 <= j - joff; j1++) {
				...
				if ((it1.info[POST2_LLD][i1 + ioff] == it1.info[POST2_LLD][i])
					...
					da = forestdist[i1 - 1][j1] + costDel;
					db = forestdist[i1][j1 - 1] + costIns;
					dc = forestdist[it1.info[POST2_LLD][i1 + ioff] - 1
							- ioff][it2.info[POST2_LLD][j1 + joff] - 1
							- joff]
							+ (switched ? delta[j1 + joff][i1 + ioff]
							: delta[i1 + ioff][j1 + joff])
							+ u;
					forestdist[i1][j1] =
							(da < db) ? ((da < dc) ? da : dc) : ((db < dc) ? db : dc);
				}
		...
		for (int i1 = 1; i1 <= i - ioff; i1++) {
			for (int j1 = 1; j1 <= j - joff; j1++) {
				...
				if ((it1.info[RPOST2_RLD][i1 + ioff] == it1.info[RPOST2_RLD][i])
					...
					double u = 0;
					u = switched
							? deltaBit[it2.info[RPOST2_POST][j1 + joff]][it1.info[RPOST2_POST][i1
							+ ioff]] * costMatch
					...
					da = forestdist[i1 - 1][j1] + costDel;
					db = forestdist[i1][j1 - 1] + costIns;
					dc = forestdist[it1.info[RPOST2_RLD][i1 + ioff] - 1
							- ioff][it2.info[RPOST2_RLD][j1 + joff] - 1
							- joff]
							+ (switched
							? delta[it2.info[RPOST2_POST][j1
							+ joff]][it1.info[RPOST2_POST][i1 + ioff]]
							: delta[it1.info[RPOST2_POST][i1
							+ ioff]][it2.info[RPOST2_POST][j1 + joff]])
							+ u;
					forestdist[i1][j1] =
							(da < db) ? ((da < dc) ? da : dc) : ((db < dc) ? db : dc);
				}
	...
	/**
	 * Computes an array where preorder/rev.preorder of a subforest of given subtree is stored and
	 * can be accessed for given i and j.
	 *
	 * @param it
	...
	private int jOfI(InfoTree it, int aI, int aSubtreeWeight, int aSubtreeRevPre, int aSubtreePre,
		...
		return aStrategy == LEFT
				...
				: aSubtreeWeight - aI - it.info[POST2_SIZE][it.info[RPOST2_POST][treeSize - 1
				- (aSubtreePre + aI)]];
	}
