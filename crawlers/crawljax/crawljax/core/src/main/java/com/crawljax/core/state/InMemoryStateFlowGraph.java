...
public class InMemoryStateFlowGraph implements Serializable, StateFlowGraph {
	...
	public List<GraphPath<StateVertex, Eventable>> getAllPossiblePathsBetween(StateVertex from, StateVertex to) {
		...
		final KShortestSimplePaths<StateVertex, Eventable> kPaths =
				new KShortestSimplePaths<>(this.sfg,
						Integer.MAX_VALUE);
		return kPaths.getPaths(from, to, Integer.MAX_VALUE);
	}
	...
	public double getPathDiversity(StateVertex s1, StateVertex s2) {
		...
		List<GraphPath<StateVertex, Eventable>> allPaths1 = getAllPossiblePathsBetween(index, s1);
		List<GraphPath<StateVertex, Eventable>> allPaths2 = getAllPossiblePathsBetween(index, s2);
		
		// calculate intersection of pair-wise event paths
		for (GraphPath<StateVertex, Eventable> eventPath1 : allPaths1){
			List<StateVertex> path1 = eventPath1.getVertexList();
			for (GraphPath<StateVertex, Eventable> eventPath2 : allPaths2){
				List<StateVertex> path2 = eventPath2.getVertexList();
				int minLength = Math.min(eventPath1.getLength() , eventPath2.getLength()) + 1;
				int intersection = 0;
				...
				sim = 2 * (double) intersection / (double)(eventPath1.getLength() + eventPath2.getLength() + 2);
				if (sim > maxSim)
					maxSim = sim;
			}
		}
