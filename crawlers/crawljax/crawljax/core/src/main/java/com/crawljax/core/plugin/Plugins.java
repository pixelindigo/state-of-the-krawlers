...
public class Plugins {
	...
	public StateVertex nextStateFromNavigationPlugin(StateFlowGraph sfg,
		CrawlPriorityMode crawlPriorityMode,
		BlockingQueue<Integer> statesWithCandidates) {
		LOGGER.debug("Running NavigationPlugins...");
		counters.get(NavigationPlugin.class).inc();
		for (Plugin plugin : plugins.get(NavigationPlugin.class)) {
			if (plugin instanceof NavigationPlugin) {
				try {
					LOGGER.debug("Calling plugin {}", plugin);
					return ((NavigationPlugin) plugin).selectState(sfg, crawlPriorityMode, statesWithCandidates);
				} catch (RuntimeException e) {
					reportFailingPlugin(plugin, e);
				}
			}
